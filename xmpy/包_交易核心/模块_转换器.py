...
class 类_持仓转换器:
    ...
    def 更新持仓(self, 持仓实例: 类_持仓数据) -> None:
        """更新持仓数据"""
        if not self.需要转换(持仓实例.合约_交易所):
            return
        持仓明细 = self.获取持仓明细(持仓实例.合约_交易所)
        持仓明细.更新持仓(持仓实例)

    def 更新成交(self, 成交实例: 类_成交数据) -> None:
        """更新成交记录"""
        if not self.需要转换(成交实例.合约_交易所):
            return
        持仓明细 = self.获取持仓明细(成交实例.合约_交易所)
        持仓明细.更新成交(成交实例)

    def 更新委托(self, 订单实例: 类_订单数据) -> None:
        """更新订单状态"""
        if not self.需要转换(订单实例.合约_交易所):
            return
        持仓明细 = self.获取持仓明细(订单实例.合约_交易所)
        持仓明细.更新委托(订单实例)

    def 更新委托请求(self, 请求: 类_订单请求, 订单标识: str) -> None:
        """处理委托请求更新"""
        if not self.需要转换(请求.合约_交易所):
            return
        持仓明细 = self.获取持仓明细(请求.合约_交易所)
        持仓明细.更新委托请求(请求, 订单标识)

    def 获取持仓明细(self, 合约_交易所: str) -> "类_持仓明细":
        """获取或创建持仓明细"""
        持仓明细 = self.持仓字典.get(合约_交易所)
        if not 持仓明细:
            合约信息 = self.获取合约信息(合约_交易所)
            持仓明细 = 类_持仓明细(合约信息)
            self.持仓字典[合约_交易所] = 持仓明细
        return 持仓明细
