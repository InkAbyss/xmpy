...
from .模块_对象 import (
    ...
    类_手续费数据, 类_订阅请求, 类_订单请求, 类_撤单请求, 类_历史数据请求, 类_报价请求, 类_K线数据
)
...
class 类_基础网关(ABC):
    ...
    def 推送行情(self, 行情实例: 类_行情数据) -> None:
        ...
        # 推送品种专属行情事件
        self.触发事件(f"{事件类型_行情}{行情实例.合约_交易所}", 行情实例)

    def 推送成交(self, 成交实例: 类_成交数据) -> None:
        """处理成交数据推送"""
        self.触发事件(事件类型_成交, 成交实例)
        self.触发事件(f"{事件类型_成交}{成交实例.合约_交易所}", 成交实例)
